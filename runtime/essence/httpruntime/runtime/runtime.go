...
func main() {
	...
	for _, schedule := range description.Schedule {
		...
		c.AddFunc(schedule.CRONInterval, func() {
			result := sdk.ScheduleInvoke(schedule, &httpruntime.ScheduleInvokeCMD{})
...
type RuntimeSDK struct {
	client      *http.Client
	address     string
	description *httpruntime.RuntimeDescription
}

func (s *RuntimeSDK) Describe() (*httpruntime.RuntimeDescription, error) {
	output := &httpruntime.RuntimeDescription{}
...
func (s *RuntimeSDK) ScheduleInvoke(schedule *httpruntime.ScheduleType, input *httpruntime.ScheduleInvokeCMD) *httpruntime.ScheduleInvokeResult {
